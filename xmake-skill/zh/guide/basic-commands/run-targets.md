...
我们也可以通过 target 的 [set\_runargs](/zh/api/description/project-target#set-runargs) 配置接口，来指定传入的运行参数，而不需要每次命令行指定它。
...
另外，我们也可以通过 target 的 [set\_rundir](/zh/api/description/project-target#set-rundir) 配置接口，来指定传入的运行参数，而不需要每次命令行指定它。
