...
如果像上面那样一个个去判断所有arm架构，也许会很繁琐，毕竟每个平台的架构类型很多，xmake提供了比[add\_files](#targetadd_files)更强的lua正则表达式匹配模式，来更加简洁的进行判断：
...
而在 msys/cygwin 子系统环境下，编译架构默认为 x86\_64/i386，是有差异的。
...
此接口与[get\_config](/zh/api/description/global-interfaces#get-config)接口配合使用，可以完整地获取和判断用户通过`xmake f --option1=xxx`设置的选项状态。
:::
...
一般配合[add\_requires](/zh/api/description/global-interfaces#add-requires)一起使用，例如：
...
::: tip 注意
此接口跟[has\_config](#has_config)的区别在于，[has\_config](#has_config)用于[option](/zh/api/description/configuration-option#option)，而它用于[add\_requires](/zh/api/description/global-interfaces#add-requires)。
:::
