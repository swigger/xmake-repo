...
本质上都是任务，只是[set\_category](#set_category)分类不同而已。
...
由于这里没有使用[set\_menu](#set_menu)设置菜单，因此这个任务只能再`xmake.lua`的自定义脚本或者其他任务内部调用，例如：
...
这是一个可选api，显示离开选项作用域，用法和[target\_end](/zh/api/description/project-target#target-end)类似。
...
如果通过[set\_category](#set_category)设置分类为`plugin`，那么这个任务就是一个插件了：
