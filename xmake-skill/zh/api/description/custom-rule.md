...
使用方式和说明请见：[target:add\_imports](/zh/api/description/project-target#add-imports)，用法相同。
...
关于这个的详细说明，见：[on\_buildcmd\_file](#on_buildcmd_file)
...
跟[on\_build\_file](#on_build_file)用法类似，不过这个接口被调用的时机是在编译某个源文件之前，
一般用于对某些源文件进行编译前的预处理。

## before\_buildcmd\_file
...
跟[on\_buildcmd\_file](#on_buildcmd_file)用法类似，不过这个接口被调用的时机是在编译某个源文件之前，
一般用于对某些源文件进行编译前的预处理。
...
跟[on\_build\_files](#on_build_files)用法类似，不过这个接口被调用的时机是在编译某些源文件之前，
一般用于对某些源文件进行编译前的预处理。

## before\_buildcmd\_files
...
跟[on\_buildcmd\_files](#on_buildcmd_files)用法类似，不过这个接口被调用的时机是在编译某些源文件之前，
一般用于对某些源文件进行编译前的预处理。
...
用于实现自定义target链接后的执行脚本，用法跟[before\_link](#before_link)类似。
...
用于实现自定义target构建后的执行脚本，用法跟[before\_build](#before_build)类似。
...
用于实现自定义target清理后的执行脚本，用法跟[before\_clean](#before_clean)类似。
...
用于实现自定义target打包后的执行脚本, 用法跟[before\_package](#before_package)类似。
...
用于实现自定义target安装后的执行脚本，用法跟[before\_install](#before_install)类似。
...
用于实现自定义target卸载后的执行脚本，用法跟[before\_uninstall](#before_uninstall)类似。
...
跟[on\_build\_file](#on_build_file)用法类似，不过这个接口被调用的时机是在编译某个源文件之后，
一般用于对某些编译后对象文件进行后期处理。

## after\_buildcmd\_file
...
跟[on\_buildcmd\_file](#on_buildcmd_file)用法类似，不过这个接口被调用的时机是在编译某个源文件之后，
一般用于对某些编译后对象文件进行后期处理。
...
跟[on\_build\_files](#on_build_files)用法类似，不过这个接口被调用的时机是在编译某些源文件之后，
一般用于对某些编译后对象文件进行后期处理。

## after\_buildcmd\_files
...
跟[on\_buildcmd\_files](#on_buildcmd_files)用法类似，不过这个接口被调用的时机是在编译某些源文件之后，
一般用于对某些编译后对象文件进行后期处理。
