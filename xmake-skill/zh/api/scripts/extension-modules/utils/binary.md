...
| opt | 可选。选项：- `plat`: 平台 - `arch`: 架构 - `recursive`: 递归获取所有依赖 (默认: false) - `resolve_path`: 解析完整路径 (默认: false) - `resolve_hint_paths`: 查找提示路径 - `resolve_search_paths`: 搜索库路径 (如 LD\_LIBRARY\_PATH) - `check_cycle`: 检测循环依赖 |
...
|------|------|
| binaryfile | 必需。输入二进制文件路径 |
| rpath\_old | 必需。旧的 rpath 路径 |
| rpath\_new | 必需。新的 rpath 路径 |
| opt | 可选。选项 |
