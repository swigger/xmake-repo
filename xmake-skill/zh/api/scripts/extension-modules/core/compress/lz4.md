...
| 类型 | 描述 |
|------|------|
| stream | 压缩流对象，支持以下方法：- `stream:write(data, opt)` - 写入数据进行压缩- `stream:read(buffer, size, opt)` - 读取压缩后的数据 |
...
| 类型 | 描述 |
|------|------|
| stream | 解压流对象，支持以下方法：- `stream:write(data, opt)` - 写入压缩数据- `stream:read(buffer, size, opt)` - 读取解压后的数据 |
