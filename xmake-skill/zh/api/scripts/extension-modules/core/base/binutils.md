...
| opt | 可选。选项：- `linewidth`: 行宽 (默认: 32) - `nozeroend`: 不追加空终止符 (默认: false) |
...
| opt | 可选。选项：- `format`: 对象格式 (coff, elf, macho) - `symbol_prefix`: 符号前缀 (默认: `_binary_`) - `arch`: 架构 (默认: x86\_64) - `plat`: 平台 (默认: macosx, 仅限 macho) - `basename`: 符号基名 - `target_minver`: 目标最小版本 (仅限 macho) - `xcode_sdkver`: Xcode SDK 版本 (仅限 macho) - `zeroend`: 追加空终止符 (默认: false) |
