...
| 事件类型 | 值 | 描述 |
|---------|-----|------|
| event.ev\_keyboard | 1 | 键盘按键按下/释放事件 |
| event.ev\_mouse | 2 | 鼠标按键/移动事件 |
| event.ev\_command | 3 | 命令事件 |
| event.ev\_text | 4 | 文本输入事件 |
| event.ev\_idle | 5 | 空闲事件（无用户输入） |
| event.ev\_max | 5 | 最大事件类型值 |
...
| 命令类型 | 值 | 描述 |
|---------|-----|------|
| event.cm\_quit | 1 | 退出应用程序 |
| event.cm\_exit | 2 | 退出当前视图 |
| event.cm\_enter | 3 | 进入/确认操作 |
| event.cm\_max | 3 | 最大命令类型值 |
...
| 参数 | 描述 |
|------|------|
| key\_name | 必需。要检查的键名（例如："Enter"、"Esc"、"Tab"） |
...
| 参数 | 描述 |
|------|------|
| command | 必需。要检查的命令名称（例如："cm\_quit"、"cm\_enter"） |
...
|------|------|
| key\_code | 必需。数字键码 |
| key\_name | 必需。键名字符串（例如："Enter"、"Esc"、"Tab"） |
| key\_meta | 必需。如果按下了 ALT 键则为 `true`，否则为 `false` |
...
| 参数 | 描述 |
|------|------|
| btn\_code | 必需。鼠标按钮事件码 |
| x | 必需。X 坐标 |
| y | 必需。Y 坐标 |
| btn\_name | 必需。按钮名称字符串 |
