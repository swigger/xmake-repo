...
这个接口的用法跟[lib.detect.find\_file](#detect-find_file)类似，唯一的区别是返回的结果不同。
...
这个接口比[lib.detect.find\_tool](#detect-find_tool)较为原始底层，通过指定的参数目录来查找可执行程序。
...
此接口也是用于查找可执行程序，不过比[lib.detect.find\_program](#detect-find_program)更加的高级，功能也更加强大，它对可执行程序进行了封装，提供了工具这个概念：
...
[lib.detect.find\_program](#detect-find_program)只能通过传入的原始program命令或路径，去判断该程序是否存在。
...
此接口跟[compiler.has\_features](/zh/api/scripts/extension-modules/core/tool/compiler#compiler-has_features)类似，但是更加原始，传入的参数是实际的工具名toolname。
...
此接口跟[compiler.has\_flags](/zh/api/scripts/extension-modules/core/tool/compiler#compiler-has_flags)类似，但是更加原始，传入的参数是实际的工具名toolname。
...
此接口是[lib.detect.check\_cxsnippets](#detect-check_cxsnippets)的简化版本，仅用于检测函数。
...
此接口跟[lib.detect.has\_cfuncs](#detect-has_cfuncs)类似，请直接参考它的使用说明，唯一区别是这个接口用于检测c++函数。

## detect.has\_cincludes
...
此接口是[lib.detect.check\_cxsnippets](#detect-check_cxsnippets)的简化版本，仅用于检测头文件。
...
此接口跟[lib.detect.has\_cincludes](#detect-has_cincludes)类似，请直接参考它的使用说明，唯一区别是这个接口用于检测c++头文件。
...
此接口是[lib.detect.check\_cxsnippets](#detect-check_cxsnippets)的简化版本，仅用于检测类型。
...
此接口跟[lib.detect.has\_ctypes](#detect-has_ctypes)类似，请直接参考它的使用说明，唯一区别是这个接口用于检测c++类型。

## detect.check\_cxsnippets
...
对于一些复杂的编译器特性，连[compiler.has\_features](/zh/api/scripts/extension-modules/core/tool/compiler#compiler-has_features)都无法检测到的时候，可以通过此接口通过尝试编译来检测它。
...
此接口是[detect.has\_cfuncs](#detect-has_cfuncs), [detect.has\_cincludes](#detect-has_cincludes)和[detect.has\_ctypes](#detect-has_ctypes)等接口的通用版本，也更加底层。
